...
class ChineseDetector(BaseDetector):
    ...
    def _analyze_ai_formal_connectives(self, text: str, score: int,
        ...
        patterns = [
            ...
            (r'这是[\u4e00-\u9fa5]{2,10}(中的|里的)?(常见|典型|普遍|基本|标准)(划分|做法|方式|模式)', '这是X常见划分'),
            (r'试图[\u4e00-\u9fa5]{2,20}(同时|一并|统一)', '试图同时X'),
            (r'容易[\u4e00-\u9fa5]{2,10}[，,]可能[\u4e00-\u9fa5]{2,10}', '容易X，可能Y'),
            (r'能够[\u4e00-\u9fa5]{1,6}且[\u4e00-\u9fa5]{1,6}地[\u4e00-\u9fa5]{2,10}', '能够X且Y地Z'),
            (r'采用[\u4e00-\u9fa5]{2,15}(操作|方法|策略|机制|方案)[\u4e00-\u9fa5]{0,6}(实现|完成|达到)', '采用X操作实现Y'),
            (r'以[\u4e00-\u9fa5]{2,8}(开销|代价|成本)[\u4e00-\u9fa5]{2,10}', '以X开销Y'),
            (r'结合了[\u4e00-\u9fa5]{2,10}和[\u4e00-\u9fa5]{2,10}', '结合了X和Y'),
            (r'借助[\u4e00-\u9fa5]{2,15}(的)?(表征|提取|感知|学习|泛化|表达)能力[\u4e00-\u9fa5]{2,10}', '借助X能力Y'),
            (r'对[\u4e00-\u9fa5]{2,10}实施[\u4e00-\u9fa5]{2,10}', '对X实施Y'),
            (r'为[\u4e00-\u9fa5]{2,10}输出[\u4e00-\u9fa5]{2,15}', '为X输出Y'),
            (r'滤除[\u4e00-\u9fa5]{2,15}(的)?(干扰|影响|噪声)', '滤除X干扰'),
            (r'规避[\u4e00-\u9fa5]{2,15}(中的)?[\u4e00-\u9fa5]{2,10}', '规避X中的Y'),
            (r'被[\u4e00-\u9fa5]{2,10}(反复|多次|不断)[\u4e00-\u9fa5]{2,10}', '被反复X'),
            (r'通过[\u4e00-\u9fa5]{2,15}(模块|结构|网络|机制|方法)[\u4e00-\u9fa5]{0,6}(进行|完成|实现)', '通过X模块进行Y'),
            (r'利用[\u4e00-\u9fa5]{2,15}(的)?(结构|模块|机制|方法|路径)[\u4e00-\u9fa5]{0,6}实现', '利用X结构实现Y'),
            (r'分别[\u4e00-\u9fa5]{0,6}(对应|适配|输入|完成|负责)', '分别对应/适配'),
            (r'该[\u4e00-\u9fa5]{2,8}(位于|处于)', '该X位于Y'),
            (r'无需[\u4e00-\u9fa5]{2,10}', '无需X'),
            (r'故将[\u4e00-\u9fa5]{2,15}', '故将X'),
            (r'相当于[\u4e00-\u9fa5]{2,20}', '相当于X'),
            (r'先经过[\u4e00-\u9fa5]{2,15}完成', '先经过X完成Y'),
            (r'其中[\u4e00-\u9fa5]{2,10}(分支|模块|部分)[\u4e00-\u9fa5]{0,6}(强化|增强|过滤|保留)', '其中X分支强化Y'),
            (r'既[\u4e00-\u9fa5]{2,15}[，,]又[\u4e00-\u9fa5]{2,15}', '既X又Y'),
            (r'这一[\u4e00-\u9fa5]{2,8}(设计|方案|方法|结构|机制)可', '这一X设计可Y'),
            (r'整合后[\u4e00-\u9fa5]{2,10}', '整合后X'),
            (r'直接[\u4e00-\u9fa5]{2,6}(接入|输入|输入到|传入)', '直接接入X'),
            (r'为[\u4e00-\u9fa5]{2,10}(验证|评估|检验|测试|确认)[\u4e00-\u9fa5]{2,10}(的)?(可用性|有效性|可行性)', '为验证X的可用性'),
            (r'完成[\u4e00-\u9fa5]{2,10}(部署|配置|搭建|安装|构建)', '完成X部署'),
            (r'构建了[\u4e00-\u9fa5]{2,15}', '构建了X'),
            (r'采用[\u4e00-\u9fa5]{2,15}(加|与|和)[\u4e00-\u9fa5]{2,15}的(架构|结构|方案)', '采用X加Y的架构'),
            (r'整个[\u4e00-\u9fa5]{2,10}流程包括', '整个X流程包括'),
            (r'可划分为[\u4e00-\u9fa5]{2,20}(四个|三个|五个|多个)阶段', '可划分为X阶段'),
            (r'被[\u4e00-\u9fa5]{2,10}(导出|转换|编译|部署)为', '被导出为X'),
            (r'降低了[\u4e00-\u9fa5]{2,10}(的)?(依赖|复杂|开销)', '降低了X的依赖'),
            (r'也提升了[\u4e00-\u9fa5]{2,10}', '也提升了X'),
            (r'一系列[\u4e00-\u9fa5]{2,10}', '一系列X'),
            (r'若[\u4e00-\u9fa5]{2,15}[，,]可通过', '若X，可通过Y'),
            (r'也可切换为[\u4e00-\u9fa5]{2,10}模式', '也可切换为X模式'),
            (r'即可启动[\u4e00-\u9fa5]{2,10}', '即可启动X'),
            (r'使用[\u4e00-\u9fa5]{2,15}构建[\u4e00-\u9fa5]{2,10}', '使用X构建Y'),
            (r'以[\u4e00-\u9fa5]{2,10}实现[\u4e00-\u9fa5]{2,10}', '以X实现Y'),
            (r'对外提供[\u4e00-\u9fa5]{2,15}', '对外提供X'),
            (r'调用[\u4e00-\u9fa5]{2,10}完成[\u4e00-\u9fa5]{2,10}', '调用X完成Y'),
            (r'将[\u4e00-\u9fa5]{2,10}后调用[\u4e00-\u9fa5]{2,10}执行', '将X后调用Y执行Z'),
            (r'适合[\u4e00-\u9fa5]{2,10}及[\u4e00-\u9fa5]{2,10}', '适合X及Y'),
            (r'默认加载[\u4e00-\u9fa5]{2,15}', '默认加载X'),
            (r'更加方便', '更加方便'),
            (r'围绕[\u4e00-\u9fa5]{2,15}面临[\u4e00-\u9fa5]{2,10}困境', '围绕X面临Y困境'),
            (r'严重制约[\u4e00-\u9fa5]{2,15}', '严重制约X'),
            (r'系统剖析[\u4e00-\u9fa5]{2,15}并识别', '系统剖析X并识别Y'),
            (r'基于此[\u4e00-\u9fa5]{2,6}[，,]', '基于此X'),
            (r'借助[\u4e00-\u9fa5]{2,15}在[\u4e00-\u9fa5]{2,10}中引入', '借助X在Y中引入Z'),
            (r'在保留[\u4e00-\u9fa5]{2,15}的同时', '在保留X的同时'),
            (r'达成[\u4e00-\u9fa5]{2,10}与[\u4e00-\u9fa5]{2,10}的均衡', '达成X与Y的均衡'),
            (r'将[\u4e00-\u9fa5]{2,15}嵌入[\u4e00-\u9fa5]{2,15}[，,]构建', '将X嵌入Y构建Z'),
            (r'理论分析证实', '理论分析证实'),
            (r'能够在[\u4e00-\u9fa5]{2,10}条件下同步', '能够在X条件下同步Y'),
            (r'诱发[\u4e00-\u9fa5]{2,10}的', '诱发X的Y'),
            (r'天然弱势', '天然弱势'),
        ]
    ...
    def _analyze_ai_causal_chain(self, text: str, score: int,
        ...
        patterns = [
            ...
            (r'采用[^，。]{2,20}(实现|完成|达到|获取|提取)', '采用X实现Y'),
            (r'使用[^，。]{2,20}(解耦|分离|拆分|划分|捕获|提取|获取)', '使用X解耦Y'),
        ]
    ...
    def _analyze_mechanical_flow(self, text: str, score: int,
        ...
        patterns = [
            ...
            (r'使用[\u4e00-\u9fa5]{2,15}(解耦|分离|拆分|划分)', '使用X解耦Y'),
            (r'[\u4e00-\u9fa5]{2,10}对应[\u4e00-\u9fa5]{2,15}[，,][\u4e00-\u9fa5]{2,10}对应[\u4e00-\u9fa5]{2,15}', 'X对应Y，X对应Y'),
            (r'[\u4e00-\u9fa5]{2,10}(分支|模块|部分)中[，,][\u4e00-\u9fa5]{2,30}(采用|使用|应用|利用)', 'X分支中采用Y'),
            (r'[\u4e00-\u9fa5]{2,10}(分支|模块|部分)在[\u4e00-\u9fa5]{2,20}(上|中)应用[\u4e00-\u9fa5]{2,15}', 'X分支在Y上应用Z'),
            (r'[\u4e00-\u9fa5]{2,15}(拼接|融合|合并|整合)[，,]结合了[\u4e00-\u9fa5]{2,15}', 'X融合，结合了Y'),
            (r'限制[\u4e00-\u9fa5]{2,15}(范围|区域|空间)', '限制X范围'),
            (r'完成[\u4e00-\u9fa5]{2,15}后[，,]与[\u4e00-\u9fa5]{2,15}(重新|再次)?(对齐|合并|融合)', '完成X后与Y重新合并'),
            (r'[\u4e00-\u9fa5]{2,10}(分支|模块)通过[\u4e00-\u9fa5]{2,15}(聚合|池化|采样)[\u4e00-\u9fa5]{2,10}', 'X分支通过聚合Y'),
            (r'[\u4e00-\u9fa5]{2,10}(分支|模块)借助[\u4e00-\u9fa5]{2,15}捕获[\u4e00-\u9fa5]{2,10}', 'X分支借助Y捕获Z'),
            (r'该[\u4e00-\u9fa5]{2,10}(设计|结构|机制|方案|方法)对[\u4e00-\u9fa5]{2,10}', '该X设计对Y'),
            (r'[\u4e00-\u9fa5]{2,10}通过[\u4e00-\u9fa5]{2,15}进行[\u4e00-\u9fa5]{2,15}', 'X通过Y进行Z'),
            (r'[\u4e00-\u9fa5]{2,10}经[\u4e00-\u9fa5]{2,15}完成[\u4e00-\u9fa5]{2,15}', 'X经Y完成Z'),
            (r'[\u4e00-\u9fa5]{2,15}生成[\u4e00-\u9fa5]{2,15}[，,]分别', 'X生成Y，分别'),
            (r'[\u4e00-\u9fa5]{2,10}(保留|保留着)[\u4e00-\u9fa5]{2,15}(的)?(分支|结构|模块)', 'X保留Y分支'),
            (r'与[\u4e00-\u9fa5]{2,15}形成[\u4e00-\u9fa5]{2,10}', '与X形成Y'),
            (r'[\u4e00-\u9fa5]{2,10}(进入|输入)[\u4e00-\u9fa5]{2,10}后', 'X进入Y后'),
            (r'[\u4e00-\u9fa5]{2,10}使用[\u4e00-\u9fa5]{2,15}构建[\u4e00-\u9fa5]{2,10}', 'X使用Y构建Z'),
            (r'[\u4e00-\u9fa5]{2,10}以[\u4e00-\u9fa5]{2,10}实现[\u4e00-\u9fa5]{2,10}', 'X以Y实现Z'),
            (r'[\u4e00-\u9fa5]{2,10}调用[\u4e00-\u9fa5]{2,10}完成[\u4e00-\u9fa5]{2,10}', 'X调用Y完成Z'),
            (r'[\u4e00-\u9fa5]{2,10}将[\u4e00-\u9fa5]{2,10}后调用[\u4e00-\u9fa5]{2,10}执行', 'X将Y后调用Z执行W'),
            (r'服务启动后[\u4e00-\u9fa5]{2,15}', '服务启动后X'),
            (r'将[\u4e00-\u9fa5]{2,15}嵌入[\u4e00-\u9fa5]{2,15}[，,]构建[\u4e00-\u9fa5]{2,10}', '将X嵌入Y构建Z'),
            (r'借助[\u4e00-\u9fa5]{2,15}在[\u4e00-\u9fa5]{2,10}中引入[\u4e00-\u9fa5]{2,10}', '借助X在Y中引入Z'),
            (r'在保留[\u4e00-\u9fa5]{2,15}的同时[\u4e00-\u9fa5]{2,15}', '在保留X的同时Y'),
        ]

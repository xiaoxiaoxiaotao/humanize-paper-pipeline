...
class VIPDetector(BaseDetector):
    ...
    VIP_AI_FINGERPRINTS = [
        ...
        r'在[^，。]{2,8}层面[，,]',
        r'不仅[^，。]{3,30}而且还[^，。]{3,30}',
        r'从而使得[^，。]{3,20}(?:得到了|获得了|实现了)',
        r'有机融合',
        r'深度融合',
        r'切实有效地',
        r'进一步(?:提升|增强|改善|强化)',
        r'本研究所积累的[^，。]{2,10}经验[，,]不仅为',
        r'逐步暴露出了[^，。]{2,15}局限性',
        r'依托[^，。]{2,10}框架[^，。]{0,10}进行了[^，。]{2,10}定义',
        r'通过为[^，。]{2,15}配备[^，。]{2,15}系统得以实现',
        r'构建了一套包含[^，。]{2,30}(?:指标|体系)',
        r'既具备[^，。]{2,20}又兼具[^，。]{2,20}',
    ]
    ...
    VIP_MECHANICAL_PATTERNS = [
        ...
        r'再次[，,][^。]{2,30}层面[，,]',
        r'最后[，,][^。]{2,30}层面[，,]',
        r'本研究(?:着手|开展|提出|引入|构建|搭建)[^，。]{5,30}',
        r'系统(?:得以|能够)实现[^，。]{3,20}顺序化执行',
        r'实验(?:分析的结果|结果)表明[，,][^。]{5,30}均(?:获得了|取得了)',
    ]

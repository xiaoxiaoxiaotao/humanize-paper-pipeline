...
"\n"
    ...
    'ai_formal_connectives': (
        ...
        "（如'即'、'，且'、'不仅...而且'、'使得'、'为X提供Y'、'意味着'、'体现出'、'相较于'、"
        "'这是X常见划分'、'试图同时X'、'容易X，可能Y'、'能够X且Y地Z'、'采用X操作实现Y'、'以X开销Y'、"
        "'借助X能力Y'、'对X实施Y'、'为X输出Y'、'滤除X干扰'、'规避X中的Y'等）。\n"
        "修复方案：替换为更自然的表达。"
        "'即'→'也就是'或'即'删除；'，且'→'，并'或拆成两句；"
        "'不仅...而且'→'既...也'或拆成两句；'使得'→'让'或'使'；"
        "'为X提供Y'→'给X提供Y'；'将X送入'→'把X输入'；"
        "'意味着'→'说明'；'体现出'→'体现'；'相较于'→'比'；"
        "'从X来看'→'从X看'；'这是X常见划分'→'这是X常见划分方式'；"
        "'试图同时X'→'想要同时X'；'能够精准且高效地'→'可以精确且高效地'；"
        "'采用X操作实现Y'→'利用X进行Y'；'以极低开销'→'用极少的开销'；"
        "'借助X的表征能力'→'利用X的表征能力'；'对X实施Y'→'对X进行Y'；"
        "'滤除X的干扰'→'去除X的干扰'；'规避X中的Y'→'避免X中的Y'；"
        "'为X输出Y'→'给X输出Y'；"
        "'通过X模块进行Y'→'用X模块来Y'；'利用X结构实现Y'→'用X结构来Y'；"
        "'分别对应/适配'→'分别负责/匹配'；'故将X'→'所以把X'；"
        "'相当于X'→'就像是X'；'既X又Y'→'既X也Y'；"
        "'整合后输出'→'合并后输出'；'无需X'→'不需要X'；'直接接入'→'直接连入'；"
        "'为验证X的可用性'→'为了验证X能不能用'；'完成X部署'→'部署好X'；"
        "'构建了X'→'搭了X'；'整个X流程包括'→'整个X流程有'；"
        "'可划分为X阶段'→'分成X步'；'被导出为X'→'导出成X'；"
        "'降低了X的依赖'→'减少了对X的依赖'；'也提升了X'→'也让X更好'；"
        "'一系列X'→'一些X'；'若X可通过Y'→'如果X可以用Y'；"
        "'也可切换为X模式'→'也能用X模式'；'即可启动X'→'就能启动X'；"
        "'使用X构建Y'→'用X搭Y'；'以X实现Y'→'用X来做Y'；"
        "'对外提供X'→'提供X'；'调用X完成Y'→'调用X来做Y'；"
        "'默认加载X'→'默认载入X'；'更加方便'→'更方便'；"
        "'围绕X面临Y困境'→'X面临Y问题'；'严重制约X'→'严重限制X'；"
        "'系统剖析X并识别Y'→'分析X并找出Y'；'基于此分析'→'因此'；"
        "'借助X在Y中引入Z'→'用X在Y中加入Z'；'在保留X的同时控制Y'→'既保留了X又控制了Y'；"
        "'达成X与Y的均衡'→'让X和Y达到平衡'；'将X嵌入Y构建Z'→'把X嵌入Y来组成Z'；"
        "'理论分析证实'→'从理论上来说'；'能够在X条件下同步优化Y'→'在X条件下同时改善Y'；"
        "'诱发的X'→'造成的X'；'天然弱势'→'天生不足'。"
    ),
    'ai_abstract_suffix': (
...
2. 【关键】加衔接词展开句子（这是区分人和AI最明显的特征）
   ...
   - "面临三重困境：A、B、C" → "遇到三个困难，分别是A、B、C"（加"分别是"展开）
   - "X的天然弱势" → "X存在的不足"（加"存在的"展开）
   - "严重制约X" → "对X产生了不利影响"（展开为完整从句）
   - "选定X后" → "在确定X后"（加"在"展开）
   - "找出其适应性缺陷" → "找出其适应性不足的地方"（加"的地方"展开）
   - "在模型中引入对X的有效抑制" → "在模型中加入了...的机制"（加"了""的机制"展开）
   - "控制X，达成Y的均衡" → "对X进行控制，以实现Y的平衡"（加"对...进行""以实现"展开）
   - "将X嵌入Y，构建Z" → "将X加入Y，从而形成Z"（加"从而"展开）
   规律：在动词前后加"来""会""的方式""对...进行""这样能够""同时""在...之后""则""分别是""存在的""的地方""的机制""以实现""从而"，让句子读起来更连贯

3. 功能词替换：书面功能词→日常动词
   - "借助"→"利用"；"实施"→删掉或改"进行"；"滤除"→"过滤"；"规避"→"防止/避免"
   - "捕获"→"捕捉/获取"；"对应"→"代表"；"精准"→"精确"
   - "该X"→"X"（删掉多余的"该"）
   - "嵌入"→"加入"；"提出"→"设计了"；"证实"→"说明"；"制约"→"限制"
   - "基于此分析"→"根据上述分析"；"达成"→"实现"；"构建出"→"从而形成"
   - "围绕X面临Y困境"→"X遇到Y困难"
   - "系统剖析X并识别Y"→"分析X并找出Y"
   - "同步优化X与Y"→"同时提高X和Y"
   - "诱发的X"→"造成的X"
...
- "本章围绕轻量化YOLO模型在工地PPE检测场景中面临三重困境：小目标特征捕获的天然弱势、复杂背景条件诱发的高漏检率与误检率、全局注意力机制的计算冗余严重制约精度与实时性的平衡"
  → "本章分析了轻量化YOLO模型在工地PPE检测场景中遇到三个困难，分别是小目标特征捕捉存在的不足、复杂背景条件造成的高漏检率和误检率、全局注意力机制带来的计算冗余对精度和实时性的平衡产生了不利影响"

- "选定YOLOv11n作为基础架构后，本文系统剖析其结构优势并识别其在工地特定场景中的适应性缺陷。基于此分析，本文提出C2HiLo轻量级特征增强模块"
  → "在确定YOLOv11n为基础架构后，本文分析其结构优势并找出该模型在工地具体场景中适应性不足的地方。根据上述分析，本文设计了C2HiLo轻量级特征增强模块"

- "该模块借助HiLo高低频特征解耦注意力机制在模型中引入对背景噪声的有效抑制。C2HiLo在保留高分辨率细节信号的同时控制计算资源消耗，达成PPE检测精度与边缘端推理效率的均衡"
  → "该模块通过HiLo高低频特征解耦注意力机制，在模型中加入了对背景噪声进行有效抑制的机制。C2HiLo在保留高分辨率细节信号的同时，对计算资源进行控制，以实现PPE检测精度和边缘端推理效率的平衡"

...
def calculate_ai_rate(text, lang):
    ...
    if lang in ["Chinese", "中文 (Chinese)"]:
        ...
        if variance_ratio < 0.25:
            uniformity_score = 30
            details['metrics']['sentence_uniformity'] = {"score": 0.8, "details": "高 (句长过于规律，缺乏参差感，建议打散)"}
        elif variance_ratio < 0.35:
            uniformity_score = 15
            details['metrics']['sentence_uniformity'] = {"score": 0.5, "details": "中等 (句长有一定规律)"}
        else:
            details['metrics']['sentence_uniformity'] = {"score": 0.1, "details": "低 (句长参差错落，呈现人类 Burstiness)"}
        score += uniformity_score
        
        # 2. Transitions (机器式过渡词)
        ai_phrases_zh = ["总而言之", "此外", "另外", "毋庸置疑", "值得注意的是", "必须指出", "不可否认", "总的来说", "综上所述", "从各个方面", "首先", "其次", "再次", "最后"]
        ...
        score += min(transition_count * 10, 30)
        
        # 3. Abstract Language (套话/空话/卖弄词/大词)
        abstract_zh = [
        ...
        score += min(abstract_count * 10, 30)
